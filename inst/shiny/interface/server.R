    ...
    edgeList <- reactive({
        ...
        if(length(myin) > 0) {
            ...
            data.frame(id = paste0("e", myin[j]), source = myin[j+1], target = myin[j+2], 
                       ...
                       target.outcome = as.numeric(myin[j + 8]), source.exposure = as.numeric(myin[j + 9]), 
                       target.exposure = as.numeric(myin[j + 10]), edge.monotone = as.numeric(myin[j + 11]))
    ...
    igraphFromList <- reactive({
        ...
        if(length(myin) > 0) {
            ...
            vertex.meta <- rbind(data.frame(vnames = edges$source, leftside = edges$source.leftside, 
                                      latent = edges$source.latent, outcome = 0, exposure = edges$source.exposure),
                                 data.frame(vnames = edges$target, leftside = edges$target.leftside, 
                                            latent = edges$target.latent, outcome = edges$target.outcome, exposure = 0))
            
            print(unique(vertex.meta))
            graphres <- graph_from_data_frame(edges[, c(1, 2, 5, 11)], vertices = unique(vertex.meta))
            
            ogleft <- V(graphres)[V(graphres)$leftside == 1]
            ogright <- V(graphres)[V(graphres)$leftside == 0]
            graphres <- add_vertices(graphres, 2, name = c("VL0", "VR0"), latent = c(1, 1), leftside = c(1, 0), outcome = c(0, 0))
            ...
            plot(graphres, vertex.color = ifelse(V(graphres)$latent == 1, "grey70", "white"), 
            ...
            legend("topleft", legend = c("latent", "outcome", "monotone edge"), pt.cex = c(3, 3, 1), 
                   pch = c(20, 22, NA), col = c("grey70", "black", "blue"), lty = c(NA, NA, 1))
