				...
				)
{
	...
	{
		printf ("ERROR: Let A be a mxn matrix.  m must be greater than n.\n");
		exit (-1);
	}
...
CTableau :: CTableau (FILE * p_pFile)
{
	...
	while (1)
	{
		if (fgets (szLine, LINE_LEN, p_pFile) == 0)
		{
			printf ("ERROR: no 'CONSTRAINTS' line\n");
			exit (0);
		}
	...
	while (1)
	{
		if (fgets (szLine, LINE_LEN, p_pFile) == 0)
		{
			printf ("ERROR: no 'OBJECTIVE' line\n");
			exit (0);
		}
		if (sscanf (szLine, "%s", szWord) >= 0)
		{
...
//		DisplayEnumRcds ();
	...
	{
		printf ("ERROR: The list is not monotonically non-increasing.\n");
		exit (-1);
	}
...
void CTableau :: DisplayEnumRcds ()
{
	...
	printf ("Contents of Enum List\n");
	pCurrent = m_pEnumList;
	while (pCurrent != NULL)
	{
		printf ("\t%6.3lf : ", pCurrent-> Value ());
...
void CTableau :: GenerateTableau (WORD * p_pNonBasics)
{
	...
	if (nBasis != m_BasisCnt)
	{
		printf ("ERROR: GenerateTableau: incorrect basis count.\n");
		exit (-1);
	}
...
void CTableau :: AddVertex (int * p_pBasisVars, double * p_pSolution)
{
	...
	{
		printf ("ERROR: Exceeded maximum number of vertices.\n");
		return;
	}
