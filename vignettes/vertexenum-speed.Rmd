...
It can handle the generalized class of problems defined in [Sachs & Gabriel & Sjölander]^[https://sachsmc.github.io/causaloptim/articles/CausalBoundsMethods.pdf]. 
...
```
```{r Multiple_Instruments_Code, eval = FALSE}
library(causaloptim)
library(igraph)
...
system.time(optimize_effect(obj))
#>     user  system  elapsed 
...
system.time(optimize_effect_2(obj))
#>  user  system  elapsed 
#> 0.139   0.001    0.140
```
Now this is acceptable! 😊 
This was achieved by modernizing a key back-end optimization algorithm and implementation. To see how it fits into the bigger picture, let's first see how `causaloptim` sets up the optimization problem.
